...
//select item.id, item_name, item_description, category_name, sub_category_name, checked_out, user_name, users.id from item join category on category_id = category.id join sub_category on sub_category_id = sub_category.id join users on user_name = users.user_name;
...
const itemsWithUsers = () => {
  return knex
    ...
    // .join("users", { user_name: "user_name" })
    .orderBy("item.id", "asc");
